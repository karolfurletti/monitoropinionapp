...
export default class Filtros{
  ...
  CountSitesEspecializados(opinions) {
    ...
    for (const opinion of opinions) {
      if (opinion.plataforma === "googlereviews" || opinion.plataforma === "yelp" || opinion.plataforma === "tripadvisor") counter += 1;
      }
        return counter;
      }
      ...
      CountRedesSociais(opinions) {
        let counter = 0;
        for (const opinion of opinions) {
          if (opinion.plataforma === "facebook" || opinion.plataforma === "twitter" || opinion.plataforma === "instagram") counter += 1;
          }
            return counter;
          }
    ...
    CountPositiveNegativePercent(opinions){
      let positive = this.CountPositiveOpinion(opinions)
      let negative = this.CountNegativeOpinion(opinions)
      ...
      let positive_percent = percent
      let negative_percent = 100 - positive_percent


      if(isNaN(positive_percent))
      positive_percent = 0
      if(isNaN(negative_percent))
      negative_percent = 0


      return [this.round(positive_percent), this.round(negative_percent)];
    }
...
FilterByYesterday(opinions){
  var inicio = moment().startOf('day').subtract(1, 'day')
  var fim =  moment().endOf('day').subtract(0,'day')
  var filteredObjects = _.filter(opinions,     
    function(each){ 
       return moment(each.data)
         .isBetween(inicio, fim) ;
...
FilterBy7days(opinions){
  var inicio = moment().startOf('day').subtract(7,'day');
  var fim =  moment().endOf('day').subtract(0,'day');
...
FilterBy30Days(opinions){
  var inicio = moment().startOf('day').subtract(30,'day');
  var fim =  moment().endOf('day').subtract(0,'day');
  var filteredObjects = _.filter(opinions,     
    function(each){ 
       return moment(each.data)
...
FilterBy3Mounts(opinions){
  var inicio = moment().startOf('month').subtract(3,'month');
  var fim =  moment().endOf('day').subtract(0,'day');
  var filteredObjects = _.filter(opinions,     
    function(each){ 
       return moment(each.data)
...
FilterByYear(opinions){
  var inicio = moment().startOf('month').subtract(1,'year');
  var fim =  moment().endOf('day').subtract(0,'day');
  var filteredObjects = _.filter(opinions,     
    function(each){ 
       return moment(each.data)
...
SplitInYear(opinions_by_cronology, estado_button_filter){
    ...
    SplitInDay(opinions_by_cronology, estado_button_filter){
      ...
      SplitInYesterday(opinions_by_cronology, estado_button_filter){
        ...
        SplitInWeek(opinions_by_cronology, estado_button_filter){
          ...
          console.log(opinions_by_cronology)
  ...
  for (let index = 0; index < splited_obj.length; index++) {
    dia = index + 1
    data.push({
      name: dia, uv:countnegative[index], pv:countpositive[index]
    })
  }
  ...
  FormatToChartFree (opinions_by_cronology, interval_init, interval_fim){
    ...
    console.log(interval_init)
    console.log(interval_fim)

    var start = moment(interval_init);
var end = moment(interval_fim);
  ...
  for (let index = 0; index < splited_obj.length; index++) {
    ...
    if(index === splited_obj.length -1){
      data.push({
        name: moment(end).format('DD MMM'), uv:countnegative[index], pv:countpositive[index]
      })
    }
...
var end = moment(interval_fim);
  ...
  for (let index = 0; index < splited_obj.length; index++) {
    ...
    if(index === splited_obj.length -1){
      data.push({
        name: moment(end).format('DD MMM'), uv:countnegative_main[index], pv:countnegative_concorrente[index]
      })
    }

    else{
      data.push({
        name: dia, uv:countnegative_main[index], pv:countnegative_concorrente[index]
      })
    }
    ...
    if(index === splited_obj2.length -1){
      data2.push({
        name: moment(end).format('DD MMM'), uv:countpositive_main[index], pv:countpositive_concorrente[index]
      })
    }

    else{
      data2.push({
        name: dia, uv:countpositive_main[index], pv:countpositive_concorrente[index]
      })
    }
